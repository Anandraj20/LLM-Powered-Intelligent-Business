...
import { ShieldCheck, Mail, Lock, LogIn, Sparkles, ArrowRight } from 'lucide-react';

export default function LoginPage() {
  const { login, loginWithGoogleMock, loading } = useAuth();
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-xs mt-1">Sign in to access your BusinessMind AI workspace</p>
        </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-800">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <button
              ...
              className="px-2.5 py-1.5 bg-purple-950/40 hover:bg-purple-900/60 text-purple-300 border border-purple-800/40 rounded-lg text-left transition font-medium"
            >
              👑 Owner
            </button>
            <button
              onClick={() => handleQuickRoleLogin('admin@businessmind.ai', 'Marcus (Admin)', 'Admin')}
              className="px-2.5 py-1.5 bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 border border-rose-800/40 rounded-lg text-left transition font-medium"
            >
              🛡️ Admin
            </button>
            <button
              onClick={() => handleQuickRoleLogin('manager@businessmind.ai', 'Sarah (Manager)', 'Manager')}
              className="px-2.5 py-1.5 bg-blue-950/40 hover:bg-blue-900/60 text-blue-300 border border-blue-800/40 rounded-lg text-left transition font-medium"
            >
              📊 Manager
            </button>
            <button
              onClick={() => handleQuickRoleLogin('sales@businessmind.ai', 'David (Sales)', 'Sales Person')}
              className="px-2.5 py-1.5 bg-amber-950/40 hover:bg-amber-900/60 text-amber-300 border border-amber-800/40 rounded-lg text-left transition font-medium"
            >
              💼 Sales Person
            </button>
            <button
              onClick={() => handleQuickRoleLogin('accountant@businessmind.ai', 'Priya (Accountant)', 'Accountant')}
              className="px-2.5 py-1.5 bg-emerald-950/40 hover:bg-emerald-900/60 text-emerald-300 border border-emerald-800/40 rounded-lg text-left transition font-medium"
            >
              📈 Accountant
            </button>
            <button
              onClick={() => handleQuickRoleLogin('employee@businessmind.ai', 'Alex (Employee)', 'Employee')}
              className="px-2.5 py-1.5 bg-slate-800/60 hover:bg-slate-800 text-slate-300 border border-slate-700/40 rounded-lg text-left transition font-medium"
            >
              👤 Employee
            </button>
          </div>
